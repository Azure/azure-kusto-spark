...
import com.microsoft.kusto.spark.datasource.{AadApplicationAuthentication, KustoOptions, WriteOptions, KustoTableCoordinates}
...
class KustoSinkTests extends FlatSpec with MockFactory with Matchers with BeforeAndAfterAll{
  ...
  private def getSink: KustoSink =
    new KustoSink(sqlContext, KustoTableCoordinates(kustoCluster, kustoDatabase, kustoTable), AadApplicationAuthentication(appId,appKey, appAuthorityId), WriteOptions(writeResultLimit = KustoOptions.NONE_RESULT_LIMIT))
