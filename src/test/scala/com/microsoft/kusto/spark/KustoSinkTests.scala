...
class KustoSinkTests extends FlatSpec with MockFactory with Matchers with BeforeAndAfterAll{
  ...
  private def getSink: KustoSink =
    new KustoSink(sqlContext, KustoTableCoordinates(kustoCluster, kustoDatabase, kustoTable), AadApplicationAuthentication(appId,appKey, appAuthorityId), KustoSparkWriteOptions(writeResultLimit = "none"))
