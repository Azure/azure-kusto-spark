...
class KustoSinkSchemaAdjustmentE2E
  ...
  private val SchemaAdjustment = "Source DataFrame schema adjustment"
  private val ColA = "ColA"
  private val ColB = "ColB"
  private val ColAColBSchema = "ColA:string, ColB:int"
  private val DynamicCsvMapping = "GenerateDynamicCsvMapping"
  private val SourceColA = "SourceColA"
  private val SourceColB = "SourceColB"
  private val WrongColB = "WrongColB"
  override def afterAll(): Unit = {
    cleanup(kustoConnectionOptions, testTablePrefix)
    spark.sparkContext.stop()
  }

  SchemaAdjustment should "not adjust" taggedAs KustoE2E in {
  ...
  SchemaAdjustment should "produce SchemaMatchException when column names not match" taggedAs KustoE2E in {
    val thrown = intercept[SchemaMatchException] {
      import spark.implicits._
      val sourceValues = (1 to expectedNumberOfRows).map(v => (newRow(v), v))
      val df = sourceValues.toDF("WrongColA", WrongColB)
      val targetSchema = ColAColBSchema
  ...
  SchemaAdjustment should "produce SchemaMatchException when source has additional columns" taggedAs KustoE2E in {
    val thrown = intercept[SchemaMatchException] {
      import spark.implicits._
      ...
      val targetSchema = ColAColBSchema
      val schemaAdjustmentMode = DynamicCsvMapping
  ...
  SchemaAdjustment should "generate dynamic csv mapping according to column names" taggedAs KustoE2E in {
    import spark.implicits._
    val sourceValues = (1 to expectedNumberOfRows).map(v => (newRow(v), v))
  ...
  SchemaAdjustment should "produce IllegalArgumentException when csvMappingNameReference in sink options" taggedAs KustoE2E in {
  ...
  SchemaAdjustment should "generate dynamic csv mapping according to column names when table does not exist " taggedAs KustoE2E in {
    import spark.implicits._
    val sourceValues = (1 to expectedNumberOfRows).map(v => (newRow(v), v))
    val df = sourceValues.toDF(SourceColA, SourceColB)
    val schemaAdjustmentMode = DynamicCsvMapping
