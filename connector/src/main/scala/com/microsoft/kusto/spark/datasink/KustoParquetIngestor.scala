...
class KustoParquetIngestor(ingestionProperties: IngestionProperties ) extends Serializable {
  ...
  def ingest(blobPath : String) : Unit = {
    val csb: ConnectionStringBuilder = ConnectionStringBuilder.createWithAadApplicationCredentials("https://ingest-sdke2etestcluster.eastus.dev.kusto.windows.net",
      "",
      "",
      "")
