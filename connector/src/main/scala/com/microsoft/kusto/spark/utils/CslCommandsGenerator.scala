...
private[kusto] object CslCommandsGenerator {
  ...
  def generateTempTableCreateCommand(
    ...
    val hiddenClause = if (hidden) {
      " with(hidden=true)"
    } else {
      ""
    }
    s".create table $tableName ($columnsTypesAndNames)$hiddenClause"
  }
  ...
  private def getFullUrlFromParams(storage: TransientStorageCredentials,storageParameters: TransientStorageParameters):String = {
    val secretString =
      if (!storage.sasDefined) {
        s""";" h@"${storage.storageAccountKey}""""
      }
      else if (storage.sasKey(0) == '?') {
        s"""" h@"${storage.sasKey}""""
      }
      else {
        s"""?" h@"${storage.sasKey}""""
  ...
  def generateExportDataCommand(
    ...
    val additionalOptionsString = additionalExportOptions
      ...
      .mkString(",", ",", "")
    // Values in the map will override,We could have chosen sizeLimit option as the default. Chosen the one in the map for consistency
    ...
    val command =
      s""".export async $compress to parquet ("${
        storageParameters.storageCredentials
          .map(sc=>getFullUrlFromParams(sc,storageParameters))
          .reduce((s, s1) => s + ",\"" + s1)
      })""" +
        s""" with ($sizeLimitOverride$nativeParquetString namePrefix="$namePrefix",
           |compressionType="$compressionFormat"$additionalOptionsString) <| $query""".stripMargin
    command
  }
  ...
  def generateTableAlterRetentionPolicy(
    ...
    val targetTableName = KustoQueryUtils.normalizeTableName(tmpTableName)
    val recoveryEnabled = if (recoverable) {
      "Enabled"
    } else {
      "Disabled"
    }
    s""".alter table $targetTableName policy retention '{ "SoftDeletePeriod": "$period", "Recoverability":"$recoveryEnabled" }' """
  }

  def generateTableAlterAutoDeletePolicy(tmpTableName: String, expiryDate: Instant): String = {
    val targetTableName = KustoQueryUtils.normalizeTableName(tmpTableName)
    s""".alter table $targetTableName policy auto_delete @'{"ExpiryDate": "${expiryDate.toString}","DeleteIfNotEmpty": true }' """
  }

  def generateShowTableMappingsCommand(tableName: String, kind: String): String = {
    val targetTableName = KustoQueryUtils.normalizeTableName(tableName)
    s""".show table $targetTableName ingestion ${kind.toLowerCase()} mappings"""
  }
