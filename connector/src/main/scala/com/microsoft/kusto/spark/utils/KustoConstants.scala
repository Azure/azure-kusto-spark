...
object KustoConstants {
  ...
  private val OneKiloByte: Int = 1024
  ...
  // TODO - make it configurable, user can then fine tune it using the SDK logs reporting fallback to queue (we should
  ...
  // Problem now is actually that the ManagedClient has a limit of 4 mb ... we need to make it configurable in the sdk for this to work
  // TODO when this is configurable - we need 3 tests: 1) max = 10 < one row size  2) row size < max=20< size(all the data), 3) size(data)< max = 4mb
