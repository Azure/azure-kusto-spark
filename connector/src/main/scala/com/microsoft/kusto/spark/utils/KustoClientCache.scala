...
object KustoClientCache {
  ...
  val adderSupplier: function.Function[ClusterAndAuth, ExtendedKustoClient] =
    new java.util.function.Function[ClusterAndAuth, ExtendedKustoClient]() {
  ...
  private def createClient(clusterAndAuth: ClusterAndAuth): ExtendedKustoClient = {
    val (engineKcsb, ingestKcsb) = clusterAndAuth.authentication match {
      ...
      case app: AadApplicationCertificateAuthentication =>
        ...
        (
          ConnectionStringBuilder.createWithAadApplicationCertificate(
            clusterAndAuth.engineUri,
            app.appId,
            keyCert.cert,
            keyCert.key,
