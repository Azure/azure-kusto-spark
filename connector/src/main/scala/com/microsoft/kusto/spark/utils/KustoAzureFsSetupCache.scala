...
private[kusto] object KustoAzureFsSetupCache {
  private val storageAccountKeyMap: mutable.Map[String, String] = mutable.Map.empty[String,String]
  ...
  def updateAndGetPrevStorageAccountAccess(account: String, secret: String, now: Instant): Boolean = {
  ...
  def updateAndGetPrevSas(container: String, account: String, secret: String, now: Instant): Boolean = {
  ...
  private[kusto] def checkIfRefreshNeeded(utcNow: Instant) = {
    Duration.between(utcNow,lastRefresh).abs().toMinutes.intValue() > KustoConstants.SparkSettingsRefreshMinutes
  }
}
