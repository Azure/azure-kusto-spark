...
object KustoReader {
  ...
  /*
   * A new native implementation of Parquet writer that uses new encoding
   * schemes was rolled out on the ADX side. This uses delta byte array for
   * strings and other byte array-based Parquet types (default in Parquet V2
   * which most modern parquet readers support by default).
   * To avoid breaking changes for applications, if the runtime is on a lower
   * version than 3.3.0 of spark runtime we explicitly set the ADX export to
   * not use the useNativeIngestion.
   * TODO - add test
   */
