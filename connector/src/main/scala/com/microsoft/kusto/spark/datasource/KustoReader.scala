...
private[kusto] object KustoReader {
  ...
  private def exportToStorage(kustoClient: Client,
    ...
    KDSU.logInfo(myName, s"Number of storage parameters = ${storage.length}")
    ...
    KDSU.logInfo(myName, s"Base export path=wasbs://${storage(0).container}@${storage(0).account}.blob.${storage(0).endpointSuffix}/$directory")
