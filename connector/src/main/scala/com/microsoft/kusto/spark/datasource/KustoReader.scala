...
private[kusto] object KustoReader {
  ...
  private[kusto] def setupBlobAccess(
    ...
    if (!KustoAzureFsSetupCache.updateAndGetPrevNativeAzureFs(now)) {
      request.sparkSession.conf.set("fs.azure", hadoopProviderClass)
      request.sparkSession.conf.set("spark.hadoop.fs.wasb.impl", hadoopProviderClass)
      request.sparkSession.conf.set("spark.hadoop.fs.azure", hadoopProviderClass)
    }
  }
